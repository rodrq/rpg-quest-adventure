...
@admin_router.get("/get_quests", dependencies=[Depends(get_current_admin_user)])
async def get_quests(db: Session = Depends(get_db)):
    quests = db.query(Quest).all()
    return quests


@admin_router.get("/get_quest/{quest_id}", dependencies=[Depends(get_current_admin_user)])
...
@admin_router.get("/get_characters", dependencies=[Depends(get_current_admin_user)])
async def get_characters(db: Session = Depends(get_db)):
    characters = db.query(Character).all()
    return characters


@admin_router.delete("/delete_character", dependencies=[Depends(get_current_admin_user)])
